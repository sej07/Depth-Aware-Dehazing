...
class EvalDepthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load depth (for reference, not used in forward)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Evaluating'):
            ...
            for i in range(dehazed.shape[0]):
                psnr = calculate_psnr(dehazed[i:i+1], clean[i:i+1])
                ssim = calculate_ssim(dehazed[i:i+1], clean[i:i+1])
